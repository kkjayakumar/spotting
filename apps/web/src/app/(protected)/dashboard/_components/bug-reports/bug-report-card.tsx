...
export function BugReportCard({
  ...
  const tags: string[] = Array.isArray(report.tags)
    ? report.tags.filter((t: unknown): t is string => typeof t === "string")
    : []
...
function VideoThumbnail({ report }: { report: BugReportListItem }) {
  const src = report.attachmentUrl
  if (!src) {
    return null
  }
