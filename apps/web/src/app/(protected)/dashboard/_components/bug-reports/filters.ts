...
export function formatStatusLabel(status: string): string {
  return (
    STATUS_OPTIONS.find((option) => option.value === (status as BugReportStatus))
      ?.label ?? status
  )
}
...
export function formatPriorityLabel(priority: string): string {
  return (
    PRIORITY_FILTER_OPTIONS.find(
      (option) => option.value === (priority as Priority)
    )?.label ?? priority
  )
}
...
export function formatVisibilityLabel(visibility: string): string {
  return (
    VISIBILITY_OPTIONS.find(
      (option) => option.value === (visibility as BugReportVisibility)
    )?.label ?? visibility
  )
}
