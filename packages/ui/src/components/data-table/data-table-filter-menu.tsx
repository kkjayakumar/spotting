...
import { DataTableRangeFilter } from "@spotting/ui/components/data-table/data-table-range-filter";
import { Button } from "@spotting/ui/components/ui/button";
import { Calendar } from "@spotting/ui/components/ui/calendar";
import {
  Command,
  CommandEmpty,
  CommandGroup,
  ...
  CommandItem,
  CommandList,
} from "@spotting/ui/components/ui/command";
import { Input } from "@spotting/ui/components/ui/input";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from "@spotting/ui/components/ui/popover";
import {
  Select,
  SelectContent,
  SelectItem,
...
import { getDefaultFilterOperator, getFilterOperators } from "@spotting/ui/lib/data-table";
import { formatDate } from "@spotting/ui/lib/format";
import { generateId } from "@spotting/ui/lib/id";
import { getFiltersStateParser } from "@spotting/ui/lib/parsers";
import { cn } from "@spotting/ui/lib/utils";
import type { ExtendedColumnFilter } from "@spotting/ui/types/data-table";
    ...
    return (
      ...
      >
        <Popover open={showFieldSelector} onOpenChange={setShowFieldSelector}>
          <PopoverTrigger render={<Button variant="ghost" size="sm" className="rounded-none rounded-l-md border border-r-0 font-normal dark:bg-input/30" />}>{columnMeta?.icon && (
...
function onFilterInputRender<TData>({
  ...
  switch (filter.variant) {
    ...
    case "multiSelect": {
      ...
      return (
        <Popover open={showValueSelector} onOpenChange={setShowValueSelector}>
          <PopoverTrigger render={<Button id={inputId} aria-controls={inputListboxId} variant="ghost" size="sm" className="h-full min-w-16 rounded-none border px-1.5 font-normal dark:bg-input/30" />}>{selectedOptions.length === 0 ? (
    ...
    case "dateRange": {
      ...
      return (
        <Popover open={showValueSelector} onOpenChange={setShowValueSelector}>
          <PopoverTrigger render={<Button id={inputId} aria-controls={inputListboxId} variant="ghost" size="sm" className={cn(
