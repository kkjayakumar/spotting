...
- API (`npm run profile:api` with default target `/v1/reports`):
...
- Worker (`npm run profile:worker`, 10 iterations):
...
- Re-run `npm run profile:api` with authenticated requests (`PROFILE_AUTH_TOKEN`, `PROFILE_ORG_ID`).
