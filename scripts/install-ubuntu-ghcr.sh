#!/usr/bin/env bash
#
# Spotting — Ubuntu server installer (GHCR pre-built images)
# Usage (from repo root, after creating .env with GHCR_IMAGE_PREFIX + SPOTTING_IMAGE_TAG):
...
if [[ "${EUID}" -ne 0 ]]; then
  echo "Run as root: sudo ./scripts/install-ubuntu-ghcr.sh" >&2
  exit 1
fi
...
load_env() {
  ...
  [[ -f "${COMPOSE_FILE}" ]] || fail "Missing ${COMPOSE_FILE} (clone the spotting repo)."
  ...
  : "${GHCR_IMAGE_PREFIX:?Set GHCR_IMAGE_PREFIX in .env (e.g. ghcr.io/your-github-owner)}"
  : "${SPOTTING_IMAGE_TAG:?Set SPOTTING_IMAGE_TAG in .env (e.g. v1.0.0)}"
  : "${NEXT_PUBLIC_SITE_URL:?Set NEXT_PUBLIC_SITE_URL in .env}"
...
start_application() {
  log "Pulling GHCR images (${GHCR_IMAGE_PREFIX}/*:${SPOTTING_IMAGE_TAG})"
  compose pull

  log "Starting Spotting containers"
  compose up -d

  log "Waiting for API (schema migrate + healthcheck)"
  for _ in $(seq 1 60); do
    if compose ps api 2>/dev/null | grep -q "(healthy)"; then
      break
    fi
    sleep 3
  done
...
verify_health() {
  ...
  printf '\nSpotting is running (GHCR tag %s).\n' "${SPOTTING_IMAGE_TAG}"
  printf '  Web:  https://%s\n' "${WEB_DOMAIN}"
  printf '  API:  https://%s/healthz\n' "${API_DOMAIN}"
  printf '\nUseful commands:\n'
  printf '  docker compose -f docker-compose.ghcr.yml --env-file .env logs -f\n'
  printf '  docker compose -f docker-compose.ghcr.yml --env-file .env restart\n'
